...
/**
 * Created by ognjen.cetkovic on 26/10/15.
 */
public class LiveSightActivity extends Activity {

    private static String LOG_TAG = LiveSightActivity.class.getSimpleName();
    ...
    /** Dynamic scale of the maneuver (minimum-maximum values) for the maneuver visibility range. Full size is 1f. **/
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        m_compositeFragment = (CompositeFragment) getFragmentManager().findFragmentById(R.id.compositefragment);
        m_compositeFragment.init(new OnEngineInitListener() {
            @Override
            public void onEngineInitializationCompleted(OnEngineInitListener.Error error) {
                if (error == OnEngineInitListener.Error.NONE) {
                    // Initialize mocked locations
                    m_currentLocation = new GeoCoordinate(52.53098, 13.38479, 0.0);   // Invalidenstrasse 117, Berlin
                    m_destination = new GeoCoordinate(52.53017, 13.38807, 0.0);       // Restaurant Honigmond, Borsigstrasse 28, Berlin
    ...
    public void onPause() {
        ...
        if (m_navigationManager != null && m_navigationManager.getRunningState() == NavigationManager.NavigationState.RUNNING) {
    ...
    public void onResume() {
        ...
        if (m_navigationManager != null && m_navigationManager.getRunningState() == NavigationManager.NavigationState.PAUSED) {
    ...
    private void startLiveSightRouting() {
        ...
        RouteManager.Error error = m_routeManager.calculateRoute(routePlan, m_routeListener);
        if (error != RouteManager.Error.NONE) {
            Log.w(LOG_TAG, "Failed to calculate route. Error: " + error);
        }
    }
    ...
    private void stopLiveSightGuidance() {
        if (m_routeManager != null) {
            m_routeManager.cancel();
        }
    ...
    private void attachNavigationListeners() {
        if (m_navigationManager != null) {
            m_navigationManager.addPositionListener(new WeakReference<NavigationManager.PositionListener>(m_navigationPositionListener));
            m_navigationManager.addNewInstructionEventListener(new WeakReference<NavigationManager.NewInstructionEventListener>(m_navigationNewInstructionListener));
            m_navigationManager.addNavigationManagerEventListener(new WeakReference<NavigationManager.NavigationManagerEventListener>(m_navigationListener));
            m_navigationManager.addRerouteListener(new WeakReference<NavigationManager.RerouteListener>(m_navigationRerouteListener));
        }
    }
    ...
    private void detachNavigationListeners() {
        if (m_navigationManager != null) {
            ...
            m_navigationManager.removeNewInstructionEventListener(m_navigationNewInstructionListener);
    ...
    private void createARWaypointerObject(GeoCoordinate waypoint) {
        ...
        // Triangle indices
        int[] vertexIndices = { 0, 1, 6, 2, 3, 5, 3, 4, 5, 7, 8, 13, 9, 10, 12, 10, 11, 12, 0, 1,
                7, 1, 8, 7, 1, 2, 8, 2, 9, 8, 2, 3, 9, 3, 10, 9, 3, 4, 10, 4, 11, 10, 4, 5, 11, 5,
                12, 11, 5, 6, 12, 6, 13, 12, 13, 6, 0, 13, 0, 7 };
    ...
    /**
     * Creates the billboard object for the specified maneuver and shows it by adding it to LiveSight.
     * Hides the previous billboard object if present.
     *
     * @param maneuver
     */
    ...
    /**
     * Creates and adds a geo-billboard object to LiveSight with a texture loaded
     ...
     */
    private void addARBillboardFromResource(GeoCoordinate location, int iconResourceId, float width) {
    ...
    /**
     * Creates and adds a geo-billboard object to LiveSight with a texture
     * loaded from a specified image.
     *
     * @param coordinate
     *            Geo-coordinate of a billboard.
     * @param img
    ...
    private void addARBillboard(GeoCoordinate coordinate, Image img, float width) {
        ...
        m_arNextManeuverBillboard.setDynamicScale(MANEUVER_VISIBILITY_RANGE, MANEUVER_DISTANCE_SCALE);
    ...
    public static int getManeuverIconResourceId(Context context, Maneuver maneuver) {
        ...
        final int iconId = maneuver.getAction() == Maneuver.Action.END ? DESTINATION_FLAG_ICON_ID : icon.ordinal();

        final String packageName = context.getPackageName();
        return context.getResources().getIdentifier("maneuver_icon_" + iconId, "drawable", packageName);
    }

    // Called on UI thread
    private final RouteManager.Listener m_routeListener = new RouteManager.Listener() {
        @Override
        ...
        public void onCalculateRouteFinished(RouteManager.Error errorCode, List<RouteResult> result) {
            // NOTE: RouteManager can send Error.NONE even when cancelled
            if (errorCode == RouteManager.Error.ROUTING_CANCELLED) {
                return;
            }
            ...
            // Error occurred
            if (errorCode != RouteManager.Error.NONE || result.size() == 0) {
                ...
                if (result.size() == 0) {
                    ...
                    Toast.makeText(getApplicationContext(), "Failed to calculate route. Error: " + errorCode, Toast.LENGTH_LONG).show();
                }
            ...
            m_liveSightMapRoute = new MapRoute(result.get(0).getRoute());
    ...
    private final NavigationManager.NavigationManagerEventListener m_navigationListener = new NavigationManager.NavigationManagerEventListener() {
        ...
        public void onEnded(final NavigationMode mode) {
            // NOTE: this method is called in both cases when destination is reached and when NavigationManager is stopped.
        ...
        public void onRouteUpdated(final Route updatedRoute) {
            ...
            Toast.makeText(getApplicationContext(), "Your route was udated!", Toast.LENGTH_LONG).show();
